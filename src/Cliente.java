...
public class Cliente extends JFrame {
    ...
    private boolean win;
    private ServidorInterface server;
    private ClienteRMI clientRMI;
    ...
    private void initInterface() {
        ...
        this.setTitle("Tsoro Yematatu  Jogador: " + playerName);
    ...
    private void connectToServer() {
        try {
            server = (ServidorInterface) Naming.lookup("rmi://localhost:2055" + "/Server");
            this.playerName = server.initPlayers();
            clientRMI = new ClienteRMI();

            String playerAddress = "rmi://localhost:2055" + "/Jogador" + playerName ;
            Naming.rebind(playerAddress, clientRMI);
            System.out.println("Jogador " + playerName + " se conectou");
            server.addressPlayer( playerName, playerAddress);
        }
        catch (Exception e) {}
    }
    ...
    private void initButtons() {
        ActionListener action = new ActionListener() {
            public void actionPerformed(ActionEvent event) {
                ...
                int piece = Integer.parseInt(b.getActionCommand());

                if (placeUsed < 3) {
                    myPieces[placeUsed] = piece;
                    placeUsed++;
                }
                turnGame++;
                piecesEnabled = false;
                if (turnGame > 3) {
                    ...
                    try {
                        server.sendPiece(piece,playerName);
    ...
    private class ClienteRMI extends UnicastRemoteObject implements ClienteInterface {
        private static final long serialVersionUID = 1L;
        ...
        public void attTurn(int piece) throws RemoteException {
            if (piece != -1) {
                textArea.append("\n##### JOGADOR " + enemyPlayer + " COLOCOU A PEÇA NA POSIÇÃO " + piece + " AGORA É SUA VEZ #####");
                try {
        ...
        public void attChat(String msg) throws RemoteException {
            ...
            if (!msg.equalsIgnoreCase("$close$")) {
                ...
                if (!msg.equalsIgnoreCase("$winer$")) {
                    textArea.append("\nJOGADOR " + enemyPlayer + " -> " + msg);

                }
                if (msg.equalsIgnoreCase("$winer$")) {
                    textArea.append("\n###### JOGADOR " + enemyPlayer + " GANHOU!!! ######");
                    piecesEnabled = false;
                ...
                if (msg.equalsIgnoreCase("!naoaguentomais")) {
                    ...
                    if (draw && playersDraw == 0) {
                        textArea.append("\n###### JOGADOR " + enemyPlayer + " NÃO AGUENTA MAIS SE VOCÊ TBM NÃO AGUENTA DIGITE !naoaguentomais  ######");
                        textArea.append("\n###### SE VOCÊ QUER CONTINUAR DIGITE !vaiaguentar ######");
                        playersDraw++;
                    } else if (draw && playersDraw == 1) {
                        textArea.append("\n###### JOGADOR " + enemyPlayer + " TAMBEM NÃO AGUENTA MAIS ######");
                        playersDraw++;
                    }
                ...
                if (msg.equalsIgnoreCase("!vaiaguentar")) {
                    textArea.append("\n###### JOGADOR " + enemyPlayer + " DISSE VAI TER QUE AGUENTAR ######");
                    draw = false;
                    playersDraw = 0;
                }
                if (msg.equalsIgnoreCase("!jogartoalha")) {
                    textArea.append("\n###### VOCÊ GANHOU!!! JOGADOR " + enemyPlayer + " JOGOU A TOALHA. ######");
                    piecesEnabled = false;
                }
            }
            try {
    ...
    private void verifyWin() {
        for (Integer[] segment : winCondition) {
            if (Arrays.asList(segment).containsAll(Arrays.asList(myPieces))) {
                win = true;
                textArea.append("\n###### JOGADOR " + playerName + " GANHOU!!! ######");
                piecesEnabled = false;

                try {
                    server.sendMessage("$winer$",playerName);
                    try {
                        textArea.setCaretPosition(textArea.getLineStartOffset(textArea.getLineCount() - 1));
                    }
                    catch (Exception e){}
    ...
    private void verifyPlay(int piece) {
        boolean isValid = false;
        Integer[] piecesToSwitch = new Integer[2];
        piecesToSwitch[0] = piece;
        for (int i = 0; i < pieces.length; i++) {
            if (pieces[i] == null) {
                piecesToSwitch[1] = i;
            }
        ...
        for (Integer[] segment : winCondition) {
            if (Arrays.asList(segment).containsAll(Arrays.asList(piecesToSwitch))) {
                isValid = true;
                for (int i = 0; i < myPieces.length; i++) {
                    if (myPieces[i].equals(piecesToSwitch[0])) {
                        myPieces[i] = piecesToSwitch[1];
                    }
                }
            }
        }
        if (isValid) {
            ...
            changeButtonPieces();
            try {
                server.sendPiece(playerName,piece);
                server.attPieces(myPieces[0], myPieces[1], myPieces[2],playerName);
            }
            catch (Exception e){}
            verifyWin();
        }
    ...
    public void sendMsgChat() {
        ...
        textArea.append("\nJOGADOR " + playerName + " -> " + msg);
        try {
            server.sendMessage(msg,playerName);
        }
        catch (Exception e){}
        ...
        if (msg.equalsIgnoreCase("!jogartoalha")) {
            textArea.append("\n###### VOCÊ JOGOU A TOALHA. JOGADOR " + enemyPlayer + " GANHOU!!! ######");
